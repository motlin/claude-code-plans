...
describe('getMemoryCountsForProjects', () => {
	...
	it('returns per-project counts grouped by project_id', () => {
		insertMemory({filePath: '/p/a/memory/1.md', projectId: 'proj-a', filename: '1.md', title: 'A1', mtimeMs: 1});
		insertMemory({filePath: '/p/a/memory/2.md', projectId: 'proj-a', filename: '2.md', title: 'A2', mtimeMs: 2});
		insertMemory({filePath: '/p/a/memory/3.md', projectId: 'proj-a', filename: '3.md', title: 'A3', mtimeMs: 3});
		insertMemory({filePath: '/p/b/memory/1.md', projectId: 'proj-b', filename: '1.md', title: 'B1', mtimeMs: 4});
...
describe('getMemoriesForProject', () => {
	it('returns rows for the requested project ordered by mtime_ms desc', () => {
		...
		insertMemory({filePath: '/p/b/memory/x.md', projectId: 'proj-b', filename: 'x.md', title: 'X', mtimeMs: 500});
	...
	it('returns an empty array when the project has no memories', () => {
		insertMemory({filePath: '/p/b/memory/x.md', projectId: 'proj-b', filename: 'x.md', title: 'X', mtimeMs: 1});
