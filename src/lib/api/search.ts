...
export const sessionSearchQueryOptions = (query: string) =>
	queryOptions({
		...
		queryFn: () => apiFetch(`/api/search/sessions?query=${encodeURIComponent(query)}`, SessionSearchResponse),
		staleTime: Infinity,
		gcTime: Infinity,
	});
...
export const messageSearchQueryOptions = (query: string) =>
	queryOptions({
		...
		queryFn: () => apiFetch(`/api/search/messages?query=${encodeURIComponent(query)}`, MessageSearchResponse),
		staleTime: Infinity,
		gcTime: Infinity,
	});
