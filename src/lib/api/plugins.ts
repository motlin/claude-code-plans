...
export function groupPluginsByMarketplace(plugins: PluginListData): PluginMarketplaceGroup[] {
	...
	for (const plugin of plugins) {
		...
		if (!group) {
			group = {
				marketplace: {id: marketplaceId, displayName: formatMarketplaceName(marketplaceId)},
				plugins: [],
...
export const pluginTreeQueryOptions = (pluginId: string) =>
	queryOptions({
		...
		queryFn: () => apiFetch(`/api/plugins/${encodeURIComponent(pluginId)}/tree`, PluginTreeResponse),
		staleTime: PLUGINS_STALE_TIME_MS,
	});
