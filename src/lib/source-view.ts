...
export async function readSourceFile(
	...
	try {
		const [content, fileStat] = await Promise.all([readFile(absolutePath, 'utf-8'), stat(absolutePath)]);
		return {
			kind,
			relativePath,
			absolutePath,
...
export function buildSourceConfig(kind: SourceKind, claudeHome: string): SourceKindConfig | null {
	switch (kind) {
		case 'plan':
			return {root: join(claudeHome, 'plans'), extension: '.md', language: 'markdown'};
		case 'memory':
			return {root: join(claudeHome, 'projects'), extension: '.md', language: 'markdown'};
		case 'task':
			return {root: join(claudeHome, 'tasks'), extension: '.json', language: 'json'};
		case 'command':
