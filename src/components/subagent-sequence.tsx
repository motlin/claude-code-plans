...
function pickTickInterval(totalMs: number): {
  intervalMs: number;
...
export function layoutSequence(agents: Subagent[]): SequenceLayout {
  const timed = agents.filter(
    (a) => toMsOrNull(a.startedAt) !== null && toMsOrNull(a.finishedAt) !== null,
  );
...
export function SubagentSequence({ agents }: { agents: Subagent[] }) {
  ...
  const columnX = (column: number) =>
    TIME_AXIS_WIDTH + PADDING_X + column * COLUMN_WIDTH + COLUMN_WIDTH / 2;
  ...
  return (
    <div className="mt-3">
                ...
                >
                  ...
                  {lane.waiting.map((range, j) => {
                    ...
                    return (
                      ...
                      >
                        <title>{`waiting ${formatDuration(range.endMs - range.startMs)}`}</title>
                      </rect>
                    );
                  })}
                </g>
      ...
      <div className="mt-2 flex items-center gap-3 text-[10px] text-t6">
        ...
        <span className="flex items-center gap-1">
          <svg width="22" height="8" className="text-accent-100">
            <line x1="0" y1="4" x2="18" y2="4" stroke="currentColor" strokeWidth="1.5" />
            <polygon points="18,1 22,4 18,7" fill="currentColor" />
          </svg>
          spawn
        </span>
        <span className="flex items-center gap-1">
