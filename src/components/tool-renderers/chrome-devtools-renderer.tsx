...
export function parseNetworkRequest(line: string): NetworkRequest | null {
  const m =
    /^reqid=(\d+)\s+(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS)\s+(\S+)\s+\[(\w+)(?:\s*-\s*(\d+))?\]/.exec(
      line.trim(),
    );
  if (!m) return null;
...
function ChromeDevtoolsContent({
  ...
  const screenshotPath = (parsedResult?.["screenshot_path"] ?? parsedResult?.["path"]) as
    | string
    | undefined;
  ...
  switch (tool) {
    case "navigate_page":
      return <NavigatePageRenderer input={input} resultText={resultText} />;
    case "new_page":
