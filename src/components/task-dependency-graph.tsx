...
export function layoutGraph(tasks: GraphTask[]): {nodes: NodePos[]; width: number; height: number} {
	...
	const edges = useMemo(() => {
		...
		for (const node of nodes) {
			for (const depId of node.task.blockedBy) {
				...
				if (dep) {
					result.push({from: dep, to: node, key: `${depId}->${node.task.taskId}`});
				}
			}
		}
		return result;
				...
				{nodes.map((n) => {
					...
					return (
						<g
							...
							style={{cursor: 'default', opacity: dimmed ? 0.3 : 1, transition: 'opacity 0.15s'}}
						>
							<rect
								x={n.x}
								y={n.y}
