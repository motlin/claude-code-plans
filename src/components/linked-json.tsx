...
function JsonValue({
	...
	switch (typeof value) {
		...
		default:
			return <span className="text-text-300">{String(value)}</span>;
	}
}
...
function resolveLink(value: string, key: string, ctx: LinkedJsonContext): ResolvedLink | null {
	...
	if (AGENT_ID_RE.test(value) && (key === 'id' || key === 'parent_agent_id' || key === 'parentAgentId')) {
		const agentSessionId = value.startsWith('agent-') ? value : `agent-${value}`;
		return {
			href: `/session/${agentSessionId}`,
			title: `Subagent session: ${agentSessionId}`,
...
function StringValue({value, path, ctx}: {value: string; path: string[]; ctx: LinkedJsonContext}) {
