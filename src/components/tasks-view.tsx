...
const DEFAULT_AGENT_COLOR = 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300';
...
function AgentCard({agent}: {agent: AgentItem}) {
	...
	const statusText = agent.status === 'error' ? 'Error' : agent.status === 'completed' ? 'Done' : 'Running';
	const statusColor =
		agent.status === 'error'
			? 'text-danger-000'
	...
	return (
		<div className="border border-border-300/15 rounded-lg overflow-hidden">
			...
			>
				...
				<span className={`px-2 py-0.5 rounded text-xs font-medium shrink-0 ${color}`}>{agent.agentType}</span>
